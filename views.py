...
def current_post(request):
  ...
  args = [post.date_posted.year, post.month(), post.day(), post.url_title()]
  return HttpResponseRedirect(reverse('blogiix.views.post_view', args=args))
...
def archive_view(request):
  ...
  for i in range(0,len(posts)):
    print posts[i].title + ' ' + str(posts[i].date_posted.year)
